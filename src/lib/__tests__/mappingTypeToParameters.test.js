...
describe("mappingTypeWithParameters", () => {
  it("should process valid data correctly", () => {
    ...
    const hostedCheckoutParams = result.get(
      PAYMENT_REQUEST_TYPES.HOSTED_CHECKOUT
    );
    expect(hostedCheckoutParams[0].jsonConfigPath).toBe(
      "createPaymentRequestV2.json"
    );
    expect(hostedCheckoutParams[1].jsonConfigPath).toBe(
      "customPaymentRequest.json"
    );
  });

  it("should handle missing JSON config column with default value", () => {
    const mockData = [
      [
        "Header1",
        "Header2",
